        ...
        dispatch_main_sync_safe(^{
            ...
            if (image) {
                if (image.size.width > self.width || image.size.height > self.height) {
                    UIImage *rescaleImage = [image rescaleImageToSize:CGSizeMake(self.width, self.height)];
                    if (rescaleImage != nil) {
                        [sself setImage:rescaleImage forState:state];
                    } else {
            ...
            dispatch_main_sync_safe(^{
                ...
                if (image) {
                    if (image.size.width > self.width || image.size.height > self.height) {
                        UIImage *rescaleImage = [image rescaleImageToSize:CGSizeMake(self.width, self.height)];
                        if (rescaleImage != nil) {
                            [sself setBackgroundImage:rescaleImage forState:state];
                        } else {
